...
echo "== initdb: create miningcore role/db + schema =="

# 1) role miningcore (idempotent)
...
# 2) database miningcore (idempotent via \gexec)
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<'SQL'
...
# 3) schema in miningcore db (idempotent)
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "miningcore" <<'SQL'
SET ROLE miningcore;
